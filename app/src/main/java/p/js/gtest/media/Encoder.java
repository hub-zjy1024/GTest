...
public class Encoder {
    ...
    /**
     ...
     @param framerate 视频帧率
     */
    public void configure2(int width, int height, Surface mSurface, int bitrate, int framerate) {
        ...
        mMC.configure(mMF, mSurface, null, MediaCodec.CONFIGURE_FLAG_ENCODE);
    }
    ...
    public int input(byte[] data,int len,long timestamp){
        ...
        Log.e("...", "dequeueInputBuffer index=" + index);
        if(index >= 0){
//            ByteBuffer inputBuffer = inputBuffers[index];
//            inputBuffer.clear();
            ByteBuffer inputBuffer = mMC.getInputBuffer(index);
...
//            outputBuffers[i].get(data, 0, mBI.size);
            ByteBuffer outputBuffers = mMC.getOutputBuffer(i);
            outputBuffers.position(mBI.offset);
            outputBuffers.limit(mBI.offset + mBI.size);
            outputBuffers.get(data, 0, mBI.size);
            len[0] = mBI.size ;
